...
describe("JumpCrossV2", function () {
  async function deploy() {
    ...
    const JCV2 = await hre.ethers.getContractFactory("JumpCrossV2");
    const jcv2 = await JCV2.deploy(jcc);

    return { provider, jcv2, jcc, owner, acc1, acc2, acc3, acc4 };
  }

  describe("Deployment", function () {
    it("State: Should set the right owner", async function () {
      const { jcv2, owner } = await loadFixture(deploy);

      expect(await jcv2.owner()).to.equal(owner.address);
    });

    it("State: Should set the right agent", async function () {
      const { jcv2, owner } = await loadFixture(deploy);
    ...
    it("State: Should set the right ERC20 contract", async function () {
      const { jcv2, jcc } = await loadFixture(deploy);

      expect(await jcv2.jcc()).to.equal(jcc);
    });
  });

  describe("Game functions", function () {
    it("Build map successfully", async function () {
      const { jcv2, owner, jcc } = await loadFixture(deploy);

      await jcc.pawn(BigInt(10000), { value: ethers.parseEther("0.14112") });
      await jcc.approve(jcv2, BigInt(10000));

      await expect(jcv2.buildMap("mockPaymentId", "sliver", BigInt(900), BigInt(300)))
        .to.emit(jcv2, "Build")
    ...
    it("Create space successfully", async function () {
      ...
      await expect(jcv2.createSpace("mockPaymentId", "mockMapId", BigInt(0), BigInt(240), BigInt(40)))
        .to.emit(jcv2, "Create")
    ...
    it("Ready at a space successfully", async function () {
      ...
      await expect(jcv2.readyAt("mockPaymentId", "mockMapId", BigInt(240), BigInt(40)))
        .to.emit(jcv2, "Ready")
    ...
    it("Upload a game result successfully", async function () {
      ...
      await expect(jcv2.upload(owner.address, "mockMapId", BigInt(177)))
        .to.emit(jcv2, "Upload")
        .withArgs(owner.address, "mockMapId", BigInt(177));
    });

    it("Settle a map successfully: (Ex: Silver, 12 joiner)", async function () {
      const { jcv2, owner, acc1, acc2, acc3 } = await loadFixture(deploy);
      ...
      await expect(jcv2.settle("mockMapId", owner.address, BuilderReward, ProtocolRevenue, winners, rewards))
        .to.emit(jcv2, "Settle")
        .withArgs("mockMapId", owner.address, BuilderReward)
        .and.to.emit(jcv2, "Share")
        .withArgs("mockMapId", ProtocolRevenue)
        .and.to.emit(jcv2, "Distribute")
  ...
  describe("Claim", function () {
    it("interation test: (Ex: Gold, 4 joiner)", async function () {
      const { jcv2, jcc, owner, acc1, acc2, acc3, acc4 } = await loadFixture(deploy);
      ...
      async function mintJCC(account: HardhatEthersSigner, amount: number, etherValue: string) {
        ...
        await jcc.connect(account).approve(jcv2, BigInt(amount));
      }
      ...
      async function playOnce(accounts: HardhatEthersSigner[]) {
        ...
        await expect(
          jcv2.connect(accounts[0]).createSpace("mockPaymentId", "mockMapId", BigInt(0), BigInt(360), BigInt(60))
        )
          .to.emit(jcv2, "Create")
          .withArgs(accounts[0].address, "mockMapId", "mockPaymentId", BigInt(0), BigInt(360), BigInt(60));

        for (let i = 1; i < accounts.length; i++) {
          await expect(jcv2.connect(accounts[i]).readyAt("mockPaymentId", "mockMapId", BigInt(360), BigInt(60)))
            .to.emit(jcv2, "Ready")
      ...
      async function settle(accounts: HardhatEthersSigner[]) {
        ...
        await expect(jcv2.settle("mockMapId", owner.address, BuilderReward, ProtocolReward, winners, rewards))
          .to.emit(jcv2, "Settle")
          .withArgs("mockMapId", owner.address, BuilderReward)
          .and.to.emit(jcv2, "Share")
      ...
      // build
      await expect(jcv2.connect(owner).buildMap("mockPaymentId", "gold", BigInt(1300), BigInt(400)))
        .to.emit(jcv2, "Build")
      ...
      // upload
      await expect(jcv2.upload(acc1.address, "mockMapId", BigInt(145)))
        .to.emit(jcv2, "Upload")
        .withArgs(acc1.address, "mockMapId", BigInt(145));

      await expect(jcv2.upload(acc2.address, "mockMapId", BigInt(167)))
        .to.emit(jcv2, "Upload")
        .withArgs(acc2.address, "mockMapId", BigInt(167));

      await expect(jcv2.upload(acc3.address, "mockMapId", BigInt(178)))
        .to.emit(jcv2, "Upload")
        .withArgs(acc3.address, "mockMapId", BigInt(178));

      await expect(jcv2.upload(acc4.address, "mockMapId", ethers.MaxUint256))
        .to.emit(jcv2, "Upload")
      ...
      await expect(jcv2.connect(owner).claim()).to.emit(jcc, "Transfer").withArgs(jcv2, owner.address, BigInt(1291));

      // simulated top 1 will get 50% from reward pool
      await expect(jcv2.connect(acc1).claim()).to.emit(jcc, "Transfer").withArgs(jcv2, acc1.address, BigInt(724));

      // simulated top 2 will get 30% from reward pool
      await expect(jcv2.connect(acc2).claim()).to.emit(jcc, "Transfer").withArgs(jcv2, acc2.address, BigInt(434));

      // simulated top 3 will get 20% from reward pool
      await expect(jcv2.connect(acc3).claim()).to.emit(jcc, "Transfer").withArgs(jcv2, acc3.address, BigInt(291));

      expect(await jcc.balanceOf(jcv2)).to.equal(BigInt(0));

      // change agent
      await expect(jcv2.connect(owner).changeAgent(ethers.ZeroAddress)).to.be.revertedWithCustomError(
        jcv2,
        "InvalidParam"
      );

      await expect(await jcv2.connect(owner).changeAgent(acc4.address)).to.not.be.reverted;
      expect(await jcv2.agent()).to.equal(acc4.address);
    });
  });
});
